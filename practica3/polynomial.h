...
bool Polynomial::IsEqual(const Polynomial& pol, const double eps) const {
  ...
  double dos = 2;
  int minimo = std::min(get_size(), pol.get_size());
  for (int i = 0; i < minimo; i++) { 
    if (fabs(get_val(i) - pol.get_val(i)) > eps) {
      differents = true;
    }
  }
  if (fabs(pol.Eval(dos) - Eval(dos)) > eps) {
    differents = true;
  }
...
bool SparsePolynomial::IsEqual(const SparsePolynomial& spol
  ...
  bool differents = false;
  double dos = 2;
  int minimo = std::min(get_nz(), spol.get_nz());
  for (int i = 0; i < minimo; i++) { 
    double indice1 = at(i).get_inx();
    double valor1 = at(i).get_val();
    double indice2 = spol.at(i).get_inx();
    double valor2 = spol.at(i).get_val();
    if ((fabs(valor1 - valor2) > eps) || (indice1 != indice2)) {
      differents = true;
    }
  }
  if (fabs(spol.Eval(dos) - Eval(dos)) > eps) {
    differents = true;
  }  
  return !differents;
}
