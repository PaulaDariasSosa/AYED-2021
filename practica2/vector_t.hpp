...
{
  assert (v.get_size() == w.get_size());   //preguntar si esto hace falta
    double producto = 0;
    for (int i = 0; i < v.get_size(); ++i) {
      double valor_v = v[i];
      double valor_w = w[i];
      producto = producto + (valor_v * valor_w);
    }
...
scal_prod(const vector_t<rational_t>& v, const vector_t<rational_t>& w)
{
  assert (v.get_size() == w.get_size());  //preguntar si esto hace falta
    double producto = 0;
    for (int i = 0; i < v.get_size(); ++i) {
      double valor_v = v[i].rational_t::value();
      double valor_w = w[i].rational_t::value();
      producto = producto + (valor_v * valor_w);
    }
    std::cout << producto << std::endl;
    return producto;
